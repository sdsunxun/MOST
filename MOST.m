%MOST V1.0
function [cout,curve,curveindex,cd] = MOST(varargin)



% Parse the input parameters
[I,High,Low,Gap_size,EP,ChordLen,T] = parse_inputs(varargin{:});
if size(I,3)==3
...
%     tic
    [corner_out curveindex cd2] = getcorner(curve,curve_mode,curve_start,curve_num,sizex,sizey,img1,ChordLen,T); 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [corners curveindex cd] = getcorner(curve,curve_mode,curve_start,curve_num,sizex,sizey,img,ChordLen,T);
    corners = [];
    cd = [];
    curveindex={};
    sig = 3.0;
    ...
    for i=1:curve_num;
        ...
        if size(xs,1)>1   
            ...
            L = curveLen+2*W;   
            tic
            C = MOSTCurvature(xs,ys,ChordLen,L,curve_mode(i,:),W); 
...
function Cd = MOSTCurvature(xs,ys,chordLen,curveLen,curve_mode,W)
...
for i=W+1:curveLen-W
    ...
    while(sum(pixel_distance(i-1-left_index:i-1,1))<chordLen)
        left_index=left_index+1;
    end
    catch exception
        i
    end
    PointA=[xs(i-1-left_index+1) ys(i-1-left_index+1)];
    dist=sum(pixel_distance(i-1-left_index+1:i-1,1));
    
    PointB=[xs(i-1-left_index) ys(i-1-left_index)]; 
    d=chordLen-dist;  %
    LeftPoint=getPointByTwoPointAndDist(PointA,PointB,d);  
    
    
    while(sum(pixel_distance(i:i+right_index,1))<chordLen)
        right_index=right_index+1;
    end
    PointA=[xs(i+right_index) ys(i+right_index)];
    dist=sum(pixel_distance(i:i+right_index-1,1));
    PointB=[xs(i+right_index+1) ys(i+right_index+1)];
    d=chordLen-dist;
    RightPoint=getPointByTwoPointAndDist(PointA,PointB,d);
   ...
   Cd=[Cd;pdist([RootPoint;MiddlePoint],'euclidean')];
   
end

function point=getPointByTwoPointAndDist(PointA,PointB,d)
...
if(dist==d)
    ...
    lambda=d/(dist-d);
    x=(PointA(1,1)+lambda*PointB(1,1))/(1+lambda);
    y=(PointA(1,2)+lambda*PointB(1,2))/(1+lambda);
...
function [I,Hg,Lo,Gap_size,EP,ChordLen,T] = parse_inputs(varargin);

error(nargchk(0,5,nargin));
Para=[3,1,0.35,0,5,0.8]; %Default experience value;
if nargin>2
    I=varargin{1};
    for i=2:nargin
